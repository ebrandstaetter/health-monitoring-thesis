...
Bereits zu Beginn der Zusammenarbeit fand eine Einschulung statt, in der uns die Richtlinien zum sicheren Umgang mit Daten, Systemen und Tools erklärt wurden.
...
Konkret wurde dabei ChatGPT als Beispiel genannt; ein öffentlich zugängliches KI-Tool, das von OpenAI betrieben wird.
Dieser Grundsatz ist aus mehreren Gründen von Bedeutung.
Erstens unterliegen Kundendaten in der Europäischen Union der Datenschutz-Grundverordnung (DSGVO), die strenge Anforderungen an die Verarbeitung, Speicherung und Weitergabe personenbezogener Daten stellt.
Werden diese Daten an Dienste übermittelt, ohne dass eine entsprechende rechtliche Grundlage vorliegt, ist dies ein Datenschutzverstoß, der für das Unternehmen erhebliche rechtliche und finanzielle Konsequenzen haben kann.
Zweitens ist das Vertrauen der Kunden in die Plattform von Hello Again sehr wichtig; jede Form von Databreach würde das Vertrauen beschädigen zwischen Kunde und Firma.
...
Ein weiterer Bestandteil der Sicherheitseinschulung war die Aufklärung über Phishing-Angriffe.
Phishing bezeichnet den Versuch, Mitarbeiter durch gefälschte E-Mails, Links oder Webseiten dazu zu bringen, sensible Informationen preiszugeben zum Beispiel Passwörter, Zugangsdaten uvm.
Für Unternehmen sind Phishing-Angriffe besonders gefährlich, weil sie häufig gezielt auf bestimmte Organisationen abgestimmt sind.
...
\subsection{Branches}
Kein direkter Zugriff auf den Main Branch:
...
Direkte Änderungen an diesem Branch durch externe würden ein Risiko darstellen, da fehlerhafte Commits Auswirkungen auf das laufende Dashboard haben könnten bzw haben.
Eigener Feature-Branch:
health-monitoring: Stattdessen wurde uns ein Branch namens health-monitoring bereitgestellt.
...
Merge in das offizielle Projekt:
...
Das Aufsetzen eines funktionsfähigen Mockup-Projekts hätte viel Zeit und Entwickler gekostet.
Da diese Devs nicht verfügbar waren, entschied man sich für einen eigenen Branch.
Diese Entscheidung hatte sowohl Vor- als auch Nachteile.
Der offensichtliche Vorteil liegt darin, dass wir mit echten Codestrukturen gearbeitet haben aber der Nachteil besteht darin, dass die Fehlertoleranz geringer ist.
Schwerwiegende Fehler im Branch hätten im schlimmsten Fall den Merge-Prozess erschwert.
